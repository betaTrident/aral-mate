...
message, process it with DeepSeek, and see a clean dashboard of tasks, deadlines, and
announcements.

## Main Features

- AI parsing with DeepSeek for school messages and parent reminders.
...
- DeepSeek API (`deepseek-chat`) through a custom fetch hook.

## File Guide

- `index.html`: Vite HTML shell and root element.
- `src/main.tsx`: React entry point that mounts the app and imports global styles.
- `src/App.tsx`: Top-level app wiring for settings, DeepSeek parsing, tasks, demo data, and dashboard state.
...
- `src/hooks/useDeepSeek.ts`: DeepSeek API integration, prompt construction, loading state, and parse errors.
...
- `package.json`, `package-lock.json`, `vite.config.ts`, `tailwind.config.js`, `postcss.config.js`, `tsconfig*.json`: Build, dependency, TypeScript, Tailwind, and Vite configuration.
